...
, m_scale{ 1.0f }, width{ 10 }, height{ 10 }, m_texture_update{ false }, m_texture_loaded{ false }, m_multiplier{ 1.0f }, m_threshold{ 0.0 }, m_threshold_mod{ false }, m_multiplier_mod{false}
{
...
void VolumeVisualizationApp::onButtonDown(const VRButtonEvent &event) {
	...
	if (event.getName() == "KbdM_Down")
	{
		m_multiplier_mod = true;
		std::cerr << "multiplier mod on" << std::endl;
	}
	if (event.getName() == "KbdT_Down")
	{
		m_threshold_mod = true;
		std::cerr << "threshold mod on" << std::endl;
	}
}
...
void VolumeVisualizationApp::onButtonUp(const VRButtonEvent &event) {
	...
	if (event.getName() == "KbdM_Up")
	{
		m_multiplier_mod = false;
		std::cerr << "multiplier mod" << std::endl;
	}
	if (event.getName() == "KbdT_Up")
	{
		m_threshold_mod = false;
		std::cerr << "threshold mod off" << std::endl;
	}
}
...
void VolumeVisualizationApp::onRenderGraphicsScene(const VRGraphicsState &renderState) {
	...
	for (int i = 0; i < m_volumes.size(); i++){
		m_volume_MV[i] = glm::make_mat4(renderState.getViewMatrix());
