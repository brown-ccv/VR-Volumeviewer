...
vec4 sample_model(sampler2D volume_texture,vec2 start_position,vec2 end_position,float ratio)
{
	...
	if(model_sample.a<.20)model_sample.a*=.1;
	// Sample transfer texture
	return texture(lut,vec2(clamp(model_sample.a,0.,1.)));
...
void main()
{
	...
	// of the eye
	if(t_hit.x<0.f)t_hit.x=max(t_hit.x,0.);
	...
	for(float step=t_hit.x;step<t_hit.y;step+=dt){
		...
		}else
		{
			...
			if(useBlend){
				...
				m_sample=(vec4(1.f-blendAlpha)*m_sample)+(vec4(blendAlpha)*sample_blend);
			}
			
			// threshold based on alpha
			m_sample.a=(m_sample.a>threshold)?m_sample.a:0.f;
			// transferfunction
			if(useLut){
				if(useMultiLut){
					m_sample.r=texture(lut,vec2(m_sample.r,.5)).r;
					m_sample.g=texture(lut,vec2(m_sample.g,.5)).g;
					m_sample.b=texture(lut,vec2(m_sample.b,.5)).b;
					m_sample.a=max(m_sample.r,max(m_sample.g,m_sample.b));
				}
				else{
					m_sample=texture(lut,vec2(clamp(m_sample.a,0.,1.),.5));
				}
			}
		...
		vFragColor.rgb+=(1.-vFragColor.a)*m_sample.a*m_sample.rgb;
		vFragColor.a+=(1.-vFragColor.a)*m_sample.a;
		
		// early exit if opacity is reached
		if(vFragColor.a>=.98){break;}
		// advance point
		dataPos+=geomDir*dt;
		
	}
	
	if(vFragColor.a<.0001f){
		discard;
	}
