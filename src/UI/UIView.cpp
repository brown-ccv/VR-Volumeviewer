...
m_render_mesh(true), m_is2D(false)
{
...
void UIView::draw_ui_callback()
{
    ...
    if (m_controller_app.is_show_menu())
    {
        ...
        if (m_is2D) {
            ImGui::SetNextWindowSize(ImVec2(544, 798), ImGuiCond_Once);
        ...
        if (ImGui::BeginTabItem("General"))
        {
            if (ImGui::Button("load file", ImVec2(ImGui::GetWindowSize().x * 0.5f - 1.5 * ImGui::GetStyle().ItemSpacing.x, 0.0f)))
            {
                ...
                m_file_extension_filter = ".txt,.nrrd";
            }
            ...
            if (numVolumes > 0)
            {
                ...
                ImGui::Checkbox("Show Mesh ", &m_render_mesh);
                ImGui::SameLine();
                ImGui::Checkbox("Show labels ", &m_render_labels);
        ...
        if (fileDialog.showFileDialog("Open File", imgui_addons::ImGuiFileBrowser::DialogMode::OPEN, ImVec2(700, 310), m_file_extension_filter))
        {
            ...
            {
                VRDataLoader::load_nrrd_file(m_controller_app, fileDialog.selected_path);
...
void UIView::render_2D(Window_Properties& window_properties)
{
    ...
    m_menu_handler->drawMenu2D(window_properties.window_w, window_properties.window_h,
        window_properties.framebuffer_w, window_properties.framebuffer_h);
}

void UIView::render_3D(glm::mat4& projection_matrix, glm::mat4& view_matrix, Window_Properties& window_properties)
{
    // render menu

    if (m_show_menu)
    {
        m_menu_handler->drawMenu3D(projection_matrix, view_matrix);
    }
}
...
bool UIView::get_render_mesh()
{
    return m_render_mesh;
}

bool UIView::get_render_labels()
{
    return m_render_labels;
}
