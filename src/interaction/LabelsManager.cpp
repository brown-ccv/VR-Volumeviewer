...
void LabelsManager::add(std::string& texture_path, float x, float y, float z, float textPosZ, float size, float offset, int volume)
{
    ...
    glm::vec3 line_start(0.0, 0.0, 0.0);
    glm::vec3 line_end(0.0, 0.0, 1.0);
    m_position.push_back(glm::vec4(0.0, 0.0, 0.0,1.0));
    m_position2.push_back(glm::vec4(0.0, 0.0, 1.0, 1.0));
...
void LabelsManager::draw_labels(glm::mat4& projection_matrix, glm::mat4& headpose, float z_scale)
{
    ...
    // draw lines
    if (m_controller_app.get_mesh_models().empty())
        return; 
    ...
    {
        if (m_controller_app.get_mesh_models()[m_billboard_labels[i].volume_id])
        {
        ...
        //glm::mat4 line_mv = glm::scale(volume_mv, glm::vec3(100.0f, 100.0f, 100.0f));
            //line_mv = glm::translate(line_mv, m_billboard_labels[0].position);
            //line_mv = glm::rotate(line_mv, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
            m_lines_shader_program.setUniform("model_view_matrix", volume_mv);

            glCheckError();
            glLineWidth(1);
            glCheckError();
            glBindVertexArray(m_billboard_labels[0].line_vba);
            glCheckError();
            ...
            glm::mat4 label_mv = glm::translate(volume_mv, m_billboard_labels[i].position);
